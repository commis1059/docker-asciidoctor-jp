...
task :pdf, %i[filepath] do |_, args|
  ...
  `asciidoctor-pdf -r asciidoctor-pdf-cjk -a pdf-style=pdf-style -a pdf-stylesdir=/styles -a pdf-fontsdir="/fonts/aozoramincho-readme-ttf;/fonts/genshingothic;/fonts/RictyDiminished-master;GEM_FONTS_DIR" -r rouge -r #{rouge_theme_path} #{args.filepath}`
end
...
task :html, %i[filepath css] do |_, args|
  ...
  `asciidoctor -a source-highlighter=highlightjs -a stylesheet=/stylesheets/#{args.css}.css #{args.filepath}`
end
...
task :reveal, %i[filepath theme] do |_, args|
  ...
  `asciidoctor-revealjs -a source-highlighter=highlightjs -a revealjsdir=https://cdnjs.cloudflare.com/ajax/libs/reveal.js/3.8.0 -a revealjs_theme=#{args.theme} #{args.filepath}`
end
